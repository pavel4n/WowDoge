...
public class DialogRequestPayment extends JDialog {
	...
	private String updatePaymentURL() {
		paymentURL = BitcoinURI.convertToBitcoinURI(textFieldToAddress.getText(), Coin.parseCoin(spinnerAmount.getValue().toString()), textFieldLabel.getText(), ""); //Utils.toNanoCoins(spinnerAmount.getValue().toString() //Version.coinNameLongSmall + ":" + textFieldToAddress.getText() + "?amount="+spinnerAmount.getValue() + "&label="+textFieldLabel.getText();
		//System.out.println(paymentURL);
		if (paymentURL.startsWith(BitcoinURI.BITCOIN_SCHEME)) {
            paymentURL = paymentURL.replaceFirst(BitcoinURI.BITCOIN_SCHEME, Version.coinNameLongSmall);
        }
		return paymentURL;
	}
