...
public class DialogSend extends JDialog {
	...
	private JComboBox comboBoxChangeAddress;
	...
	public void setMaximum(Coin max) {//float max
		spinnerAmount.setModel(new SpinnerNumberModel(new Float(0), new Float(0), new Float(max.toPlainString()), new Float(1f))); //max
	...
	public Coin getAmount() {
		return (Coin.parseCoin(spinnerAmount.getValue().toString()));
	}
	
	public void setAmount(Coin amount) {
		spinnerAmount.setValue(new Float(amount.toPlainString()));
	} //new Float(amount)
	
	public Address getChangeAddress() {
		int index = comboBoxChangeAddress.getSelectedIndex();
		if (index == -1)
			return null;
		else
			return new Address(networkParams, this.changeAddresses.get(index));
	}
	
	public void setChangeAddresses(List<String> addresses) {
		this.changeAddresses = addresses;
		AddressesComboBoxModel addressesComboBoxModel = new AddressesComboBoxModel();
		addressesComboBoxModel.setAddresses(addresses);
		comboBoxChangeAddress.setModel(addressesComboBoxModel);
		if (addresses.size() > 0) {
			comboBoxChangeAddress.setSelectedIndex((int)(Math.random() * addresses.size()));
		}
	}
...
//	}
	...
	public DialogSend() {
		...
		contentPanel.setLayout(new BorderLayout(0, 0));
		{
			...
			}
			{
				...
				panel.setLayout(new BorderLayout(0, 0));
				{
					...
					textFieldToAddress.setToolTipText("Specify recipient address");
				...
				}
				{
					...
					spinnerAmount.setToolTipText("Specify amount");
				...
				}
				{
					...
					panel.add(scrollPane, BorderLayout.CENTER);
					{
						...
						btnScan.setToolTipText("Scan recipient address");
					...
					}
					{
						...
						btnDonate.setToolTipText("If you wish to donate...");
					}
		...
		okButton.setToolTipText("Send " + Version.coinNameLongSmall + "s");
		...
		{
			JPanel panel = new JPanel();
			panel.setToolTipText("Where rest of the coins from transaction should be stored?");
			panelAmount.add(panel, BorderLayout.CENTER);
			panel.setLayout(new BorderLayout(0, 0));
			{
				comboBoxChangeAddress = new JComboBox();
				comboBoxChangeAddress.setEnabled(false);
				comboBoxChangeAddress.setToolTipText("Where rest of the coins from transaction should be stored?");
				panel.add(comboBoxChangeAddress, BorderLayout.CENTER);
			}
			{
				JCheckBox chckbxChangeAddress = new JCheckBox("(Optional) Change Address Specification");
				chckbxChangeAddress.addItemListener(new ItemListener() {
					public void itemStateChanged(ItemEvent e) {
						comboBoxChangeAddress.setEnabled(e.getStateChange() == java.awt.event.ItemEvent.SELECTED);
					}
				});
				chckbxChangeAddress.setToolTipText("Optionally specify change address where the rest of the coins from transaction should be stored");
				chckbxChangeAddress.setHorizontalAlignment(SwingConstants.CENTER);
				panel.add(chckbxChangeAddress, BorderLayout.NORTH);
			}
		}
		{
